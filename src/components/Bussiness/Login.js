                    ...
                    async function signIn() {
                        try {
                            ...
                            if (currentUser && currentUser.emailVerified === false) {
                                err = "Account not verified, a link was sent to your email";
                                setError(err);
                                setSubmitting(false);
                                return;
                            }
                            ...
                            if (
                                error &&
                                error.message.toLowerCase() === "Firebase: Error (auth/user-not-found).".toLowerCase()
                            ) {
                                err2 = {
                                    message: "Not an exsiting user, please create an account",
                                };
                            } else if (
                                error &&
                                error.message.toLowerCase() === "Firebase: Error (auth/wrong-password).".toLowerCase()
                            ) {
                                err2 = {
                ...
                {({ values, errors, touched, handleChange, handleSubmit, isSubmitting }) => (
                    <form onSubmit={handleSubmit}>
                        <div className={`input-business ${errors.email && touched.email && "error errmsg"} `}>
                            <p className={"text text-small input-label"}>Email</p>
                        ...
                        <p onClick={() => history.push("/reset-password")} className="text text-small text-right">
                            Forgot Password?
                        </p>
